      ...
      <@markup id="form">
         <form id="${el}-form" accept-charset="UTF-8" method="post" action="${loginUrl}" class="form-fields login ${edition}">
            ...
            <div class="form-field">
               ...
               <script type="text/javascript">//<![CDATA[
                  function ucmLoginAsGuest(userConfigResponse) {
                     document.getElementById('${el}-username').value = userConfigResponse.json.username;
                     document.getElementById('${el}-password').value = userConfigResponse.json.password;
                     document.getElementById('${el}-form').submit();
                  }
               
                  function ucmHandleLoginAsGuestClick() {
                     var successCallback = {fn: ucmLoginAsGuest, scope: this};
                     var config = {url: '${url.context}/proxy/alfresco-noauth/ucm/anonymous-user', successCallback: successCallback, failureMessage: "${msg("login.guest.failure")}"};
                     Alfresco.util.Ajax.jsonGet(config);
                     return false;
                  }
               //]]></script>
               <span class="yui-button yui-submit-button ucm-submit-button" id="${el}-submit-guest">
                  <span class="first-child">
                     <button type="button" id="${el}-submit-guest-button" onclick="return ucmHandleLoginAsGuestClick();">${msg("button.login.guest")}</button>
                  </span>
               </span>
            </div>
            </@markup>
